...
export async function getTeamMembers() {
  try {
    console.log("KV_REST_API_URL:", KV_REST_API_URL); // <-- Add this
    console.log("KV_REST_API_TOKEN:", KV_REST_API_TOKEN ? "Set" : "Not Set"); // <-- Add this

    const data = await fetchData('team');
    console.log("Data received from fetchData", data);
    // If data is null or undefined, return an empty array instead of crashing
    return data || [];
  } catch (error) {
    console.error("Error in getTeamMembers:", error);
    // If fetchData itself throws an error, return an empty array
    return [];
  }
}